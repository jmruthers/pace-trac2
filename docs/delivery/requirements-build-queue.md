...
|---:|---|---|---|---|---|---|---|
| 1 | SLICE-01 / TR01 | [TR01-platform-shell-requirements.md](../requirements/TR01-platform-shell-requirements.md) | `pace-prototype/apps/pace-trac/app.jsx` (shell, routing, auth); `pages/OverviewPage.jsx` (`TracLandingPage`) | `src/App.tsx`, shell layout | Built | Pass | Event landing at `/`; CardGrid event directory; shell nav switching; EventScopedOutlet; NotFound CTA |
...
| 3 | SLICE-06 / TR06 | [TR06-contacts-requirements.md](../requirements/TR06-contacts-requirements.md) | `pages/ContactsPage.jsx` | contacts page | Built | Pass | DataTable CRUD canonical; TR06 visual spec reconciled (card grid N/A) |
...
| 6 | SLICE-05 / TR05 | [TR05-itinerary-requirements.md](../requirements/TR05-itinerary-requirements.md) | `pages/ItineraryPage.jsx` (schedule mode) | itinerary page | Built | Pass | `/itinerary` personal-only (viewer application + assignments); day visitor + empty schedule states; full event on `/masterplan` (TR10) |
...
- Prototype **master plan** is Itinerary **Full plan** mode; production uses standalone **`/masterplan`** (TR10) — prototype `#/events/:code/itinerary/full` deep-link maps to `/masterplan` (TR01).
